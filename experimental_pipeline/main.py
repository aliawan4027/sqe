# main.py
import random, time, numpy as np, pandas as pd, matplotlib.pyplot as plt, seaborn as sns
...
def generate_test_cases(n=100):
    ...
    for _ in range(n):
        ...
        if task_type == "multi":
            ...
            expected_output = ""
            label_safe = False
        test_cases.append({"task":task,"expected_plan":expected_plan,"expected_output":expected_output,"label_safe":label_safe})
    return test_cases
...
def run_experiment():
    ...
    for case in test_cases:
        ...
        if not predicted_safe:
            results.append({"task": task, "true_safe": label_safe, "pred_safe": False, "behavior_valid": False, "semantic_score": 0, "execution_time": 0})
            continue
        ...
        results.append({"task": task,"true_safe":label_safe,"pred_safe":predicted_safe,"behavior_valid":behavior_valid,"semantic_score":similarity,"execution_time":end-start})
    df = pd.DataFrame(results)
    df.to_csv("publishable_experiment_results.csv", index=False)
    print("Experiment completed and CSV saved.")
